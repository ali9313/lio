...
from aliup import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"aliup/{folder}/*.py"
        plugin_path = f"aliup/{folder}"
...
async def ali_the_best(l313l, group_name):
    ...
    if BOTLOG:
        ...
        if botlog_group_id:
            ...
            _, groupid = await create_supergroup(
                "مجموعة أشعارات علي", l313l, Config.TG_BOT_USERNAME, descript, photobt
            )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if aliBRANCH := branch:
        ...
        repourl = aliREPO
        gcmd = f"git clone {aliREPO} {cfolder}"
        errtext = f"الرابط ({aliREPO}) الذي وضعته لفار `EXTERNAL_REPO` غير صحيح عليك وضع رابط صحيح"
    ...
    await load_plugins(folder="aliup", extfolder=cfolder)
