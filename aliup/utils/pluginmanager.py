...
from aliup import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        checkplugins(path)
        name = "aliup.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"aliup/plugins/{shortname}.py")
            name = f"aliup.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["aliup.plugins." + shortname] = mod
        LOGS.info("تم بنجاح تحميل ملف " + shortname)
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"aliup/plugins/assistant/{shortname}.py")
        name = "aliup.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["aliup.plugins.assistant" + shortname] = mod
