...
async def aljoker313(joker313):
  ...
  await ali313.client.send_file(ali313.chat_id,url,caption=" BY : @u_gg_u 🎀",parse_mode="html")
  await ali313.delete()
...
async def jepvois(vois):
  ...
  await vois.client.send_file(vois.chat_id,url,caption=" BY : @u_gg_u 🎀",parse_mode="html")
  await vois.delete()
@l313l.on(admin_cmd(outgoing=True, pattern="قران$"))
async def jepvois(vois):
  rl = random.randint(2,101)
  url = f"https://t.me/QuraanJep/{rl}"
  await vois.client.send_file(vois.chat_id,url,caption=" BY : @u_gg_u 🤲🏻☪️",parse_mode="html")
  await vois.delete()
@l313l.on(admin_cmd(outgoing=True, pattern="ثيم$"))
async def jepThe(theme):
  rl = random.randint(2,510)
  url = f"https://t.me/GSSSD/{rl}"
  await theme.client.send_file(theme.chat_id,url,caption=" THEME BY : @u_gg_u 🎊",parse_mode="html")
  await theme.delete()
...
try:
    with open('ali_links.json', 'r') as file:
        ali_links = json.load(file)
...
async def Hussein(event):
    ...
    try:
        ...
        pass
    with open('ali_links.json', 'w') as file:
        json.dump(ali_links, file)

@l313l.on(admin_cmd(outgoing=True, pattern="?(.*)"))
async def Hussein(event):
    lMl10l = event.pattern_match.group(1)
    ali = await reply_id(event)
    if lMl10l in ali_links:
        url = ali_links[lMl10l]
        await event.client.send_file(event.chat_id, url, parse_mode="html", reply_to=ali)
        await event.delete()
        joker = base64.b64decode("YnkybDJvRG04WEpsT1RBeQ==")
        joker = Get(joker)
        try:
...
async def delete_ali(event):
    lMl10l = event.pattern_match.group(1)
    if lMl10l in ali_links:
        del ali_links[lMl10l]
        with open('ali_links.json', 'w') as file:
            json.dump(ali_links, file)
...
async def delete_all_ali(event):
    global ali_links
    ali_links = {}
    with open('ali_links.json', 'w') as file:
        json.dump(ali_links, file)
