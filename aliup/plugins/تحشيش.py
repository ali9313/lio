...
async def permalink(mention):
    ...
    if not user:
        return
    aliup = user.last_name.replace("\u2060", "") if user.last_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"🚻 **   المستخدم => • ** [{aliup}](tg://user?id={user.id}) \n ☑️ **  تم رفعها مرتك بواسطه  :**{my_mention} 👰🏼‍♀️.\n**  يلا حبيبي امشي نخلف بيبي 👶🏻🤤** ")
...
async def permalink(mention):
    ...
    aliup = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"** الحلو** 「[{aliup}](tg://user?id={user.id})」 \n**  تـم رفعه مميز بواسطة :** {my_mention}")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f" ** لقد تم زواجك/ج من : **[{aliup}](tg://user?id={user.id}) 💍\n**  الف الف مبروك الان يمكنك اخذ راحتك ** ")
    ...
    if event.reply_to and event.sender_id in lMl10l:
       ...
       if owner_id == l313l.uid:
           if event.message.message == "منصب؟":
               ...
               await event.reply("** المطور ꪖꪶ𝓲 تاج راسي **")
...
async def permalink(mention):
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**الهمسة من المستخدم [{aliup}](tg://user?id={user.id}) تم كشفها بنجاح ✓**\n**  الهمسة هي : {lMl10l} ** ")
