...
from aliup import HEROKU_APP, UPSTREAM_REPO_URL, l313l
...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"** قام مطورين السورس بتحديث التمساح**\n **التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "**  تم تحديث سورس التمساح بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "** يـتـم البـحـث عـن تـحديثـات سـورس التمساح انـتـظـر**")
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس التمساح ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("**  جار تحـديـث سـورس التمساح انـتـظـر قـليـلا 🔨**")
...
async def Hussein(event):
    ...
    event = await edit_or_reply(event, "** جارِ تحديث ريبو التنصيب لسورس التمساح**")
    off_repo = "https://github.com/ali9313/test"
    os.chdir("/app")
    try:
        txt = (
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit("** جارِ اعادة تنصيب سورس التمساح, انتظر قليلاً ..**")
...
async def reda(event):
    ...
    if event.message.message == "تحديث اجباري" and event.sender_id in progs:
        ...
        if conf == "الان":
            await event.edit("**  يتم تحديث سورس التمساح بامر المطور اجبارياً**")
            await update(event, repo, ups_rem, ac_br)
