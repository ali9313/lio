...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"** قام مطورين السورس بتحديث ꪖꪶ𝓲**\n **التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "**  تم تحديث سورس ꪖꪶ𝓲 بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, "** يـتـم البـحـث عـن تـحديثـات سـورس ꪖꪶ𝓲 انـتـظـر**")
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس ꪖꪶ𝓲 ارسل : `.تحديث الان` "
        )
    ...
    if conf == "الان":
        await event.edit("** ᯽︙ جار تحـديـث سـورس ꪖꪶ𝓲 انـتـظـر قـليـلا 🔨**")
...
async def Hussein(event):
    ...
    event = await edit_or_reply(event, "** جارِ تحديث ريبو التنصيب لسورس ꪖꪶ𝓲 **")
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit("** جارِ اعادة تنصيب سورس ꪖꪶ𝓲, انتظر قليلاً ..**")
...
async def reda(event):
    ...
    if event.message.message == "تحديث اجباري" and event.sender_id in progs:
        ...
        if conf == "الان":
            await event.edit("**  يتم تحديث سورس ꪖꪶ𝓲 بامر المطور اجبارياً**")
            await update(event, repo, ups_rem, ac_br)
