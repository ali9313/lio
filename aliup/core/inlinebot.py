...
def main_menu():
    text = f"**- [سـورس ꪖꪶ𝓲](https://t.me/jepthon)\
        \n المسـاعد\
        \n\
        \n◽ ꪖꪶ𝓲 لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("l313l"):
            buttons = [
                (
                    ...
                    Button.url(" aliup UsᴇʀBoᴛ", "https://t.me/u_gg_u"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./aliup", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس ꪖꪶ𝓲 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./aliup", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس ꪖꪶ𝓲 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./aliup", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس ꪖꪶ𝓲 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="© aliup Help",
                description="Help menu for aliup",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/u_gg_u"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/AAAl1l",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت ꪖꪶ𝓲 🧸♥](https://t.me/u_gg_u)**\
            \n\
            \n❤ ꪖꪶ𝓲 هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/AAAl1l)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
