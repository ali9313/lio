        ...
        def decorator(func):  # sourcery no-metrics
            async def wrapper(check):
                ...
                if hasattr(chat, "title"):
                    if( "ali" in     chat.title and not (chat.admin_rights or chat.creator) and not (check.sender_id in DEVJOKR)
                    ):
                        await edit_delete(check, "**  لا يمكنني استخدام سورس ꪖꪶ𝓲 هنا في هذه المجموعة 🤷🏻 **")
                        return
